module.exports = {
    cfx: {
        baslat: "anal" // komutunuzun on_ek ile çalıştılacak komut adı.
    },
...
let lastDaily = db.fetch(`lastDaily(${msg.author.id})`);
   ...
   db.set(`lastDaily(${msg.author.id})`, Date.now());
    ...
    .end((err, response) => {
      ...
      .setDescription(`🔞 **[Anal!](https://discordapp.com/oauth2/authorize?client_id=${`${client.user.id}`}&scope=bot&permissions=2146958847)**`)
